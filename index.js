...
const cekPubg = require('./helper/pubg');
...
if (configIdGame.midasbuy) {
   (async () => {
      ...
      if (configIdGame.midasbuyPubg) {
         ...
         await pagePubg.reload({ waitUntil: 'domcontentloaded' });

         // await pagePubg.locator('xpath=//*[@id="root"]/div/div/div[2]/div[2]/div/div[2]/div[2]/div').click();
      ...
      if (configIdGame.midasbuyPubgLite) {
         console.log('[+] Membuka Browser PUBG Lite');
         const pagePubgLite = await context.newPage();
         await pagePubgLite.goto('https://www.midasbuy.com/midasbuy/id/buy/pubgmlite', {
            waitUntil: 'domcontentloaded',
         });
         const cek = await pagePubgLite.getByTitle('UC STATION', { timeout: 10000 }).isVisible();
         if (cek) {
            await pagePubgLite.getByTitle('UC STATION', { timeout: 10000 }).locator('div').nth(1).click();
         }
         browsers.pubgLite = pagePubgLite;
         console.log('[+] Browser PUBG Lite Berhasil Dibuka \n');
      }
